    ...
    @notification = {
		...
		if @allowed 
			unless friends_added
				render json: { error: "You don't have friends to Say Hello to. Start making new friends first or add them by username.", contact_is_live: false, is_success: false}, status: :ok
			else
			  case can_call
					when true
